...
export class CartController {
    ...
    addToCart = catchAsync(async (req, res) => {
        ...
        return res.status(StatusCodes.OK).send(new ApiResponse(StatusCodes.OK, cart, 'Product Added To successfully'))
    })
    incrementProductCount = catchAsync(async (req, res) => {
        const { cartId } = req.params
        if (!req?.user) throw new AppError(StatusCodes.UNAUTHORIZED, 'Token Missing in Headers')
        const cart = await this.service.incrementProductQuantity(cartId, req.user)
        return res.status(StatusCodes.OK).send(new ApiResponse(StatusCodes.OK, cart, 'Product Quantity Increased successfully'))
    })
    decrementProductCount = catchAsync(async (req, res) => {
        const { cartId } = req.params
        if (!req?.user) throw new AppError(StatusCodes.UNAUTHORIZED, 'Token Missing in Headers')
        const cart = await this.service.decrementProductQuantity(cartId, req.user)
        return res.status(StatusCodes.OK).send(new ApiResponse(StatusCodes.OK, cart, 'Product Quantity Decreased successfully'))
    })
    removeProduct = catchAsync(async (req, res) => {
        ...
        return res.status(StatusCodes.OK).send(new ApiResponse(StatusCodes.OK, cart, 'Product Removed From Cart successfully'))
    })
    ...
    clearCart = catchAsync(async (req, res) => {
        if (!req?.user) throw new AppError(StatusCodes.UNAUTHORIZED, 'Token Missing in Headers')
        const cart = await this.service.emptyCart(req.user)
        return res.status(StatusCodes.OK).send(new ApiResponse(StatusCodes.OK, cart, 'cart cleared successfully'))
    })
}
